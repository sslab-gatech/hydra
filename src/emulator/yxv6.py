...
def _init_yxv6_img():
    inode = c.Inode(id=0, name=".", type=c.DIR, mode=0755, size=2134016)
    c.DISK[inode.id] = inode
    c.DENTRY["."] = inode.id

    inode = c.Inode(id=2, name="foo", type=c.DIR, mode=0755, size=2134016)
    c.DISK[inode.id] = inode
    ...
    inode = c.Inode(id=3, name="bar", type=c.DIR, mode=0755, size=2134016)
    c.DISK[inode.id] = inode
    c.DENTRY["./foo/bar"] = inode.id
    tup = (inode.id, "bar")
    tup_parent = (2, "foo")
    c.DISK[2].add_child(tup)
    c.DISK[2].persist_children()
    c.FT.add_node(tup, tup_parent)

    inode = c.Inode(id=7, name="baz", type=c.FILE, mode=0644, size=12)
    inode.datablock.data = "hello world\n"
    c.DISK[inode.id] = inode
    c.DENTRY["./foo/bar/baz"] = inode.id
    tup = (inode.id, "baz")
    tup_parent = (3, "bar")
    c.DISK[3].add_child(tup)
    c.FT.add_node(tup, tup_parent)
...
if __name__ == "__main__":
    ...
    if args.regtest:
        c.REGTEST = 1
    if args.prog:
        prog = args.prog # will be given by yxv6-cc
    else:
    ...
    if args.monochrome:
        ...
        c.color_end = ""
    if args.mnt_point:
        mnt_point = args.mnt_point # will be given by yxv6-cc
    else:

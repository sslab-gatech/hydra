...
void hfsplus_fuzzer::fix_checksum() 
{
    ;
}

void hfsplus_fuzzer::fix_general_checksum() 
{
    ;
}

extern void find_meta(std::set<uint64_t> &meta_blocks, const char *src);
void hfsplus_fuzzer::compress(
    const char *in_path,
    void *buffer,
...
void hfsplus_fuzzer::decompress(
...
void hfsplus_fuzzer::general_decompress(
    const void *meta_buffer,
    size_t meta_len,
    bool checksum) {

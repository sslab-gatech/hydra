...
int main(int argc, char *argv[])
{
  ...
  if (!strcmp(argv[1], "test")) {
    ...
    btrfs_fuzzer.compress(in_path, image_buffer, meta_path);
    ...
    btrfs_fuzzer.decompress(tmp_buffer, st.st_size);
    ...
    void *image_buffer = malloc(st.st_size);
    btrfs_fuzzer.compress(in_path, image_buffer);
    ...
    btrfs_fuzzer.decompress(tmp_buffer, st.st_size, true);

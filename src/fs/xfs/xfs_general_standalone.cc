...
/* usage: 
   ./xfs_standalone test in_image out_image meta_image
...
int main(int argc, char *argv[]) 
{
  ...
  if (!strcmp(argv[1], "test")) {
    ...
    xfs_fuzzer.general_compress(in_path, image_buffer, meta_path);
    ...
    xfs_fuzzer.general_decompress(tmp_buffer, st.st_size, true);

    xfs_fuzzer.sync_to_file(out_path);
    ...
    void *image_buffer = malloc(st.st_size);
    xfs_fuzzer.general_compress(in_path, image_buffer, "meta.img");

...
static const cmdinfo_t	metadump_cmd =
	{ "fuzz_metadump", NULL, metadump_f, 0, -1, 0,
		N_("[-a] [-e] [-g] [-m max_extent] [-w] [-o] filename"),
		N_("dump metadata to a file"), metadump_help };

static FILE		*outf;		/* metadump file */
static FILE   *outcrc;  /* crc information */
...
write_index(void)
{
	...
	if (fwrite((char *)metablock + 8, (1 << BBSHIFT) - 8, 1, outf) != 1) {
	...
	iocur_t		*buf)
{
  ...
  if (xfs_sb_version_hascrc(&mp->m_sb) && bp && bp->b_ops) {

    unsigned long bn = bp->b_bn; 
    unsigned long blen = bp->b_length;
    unsigned long bcount = bp->b_bcount;
    char *bname = bp->b_ops->name;
    
    printf("writing blocks at %s bno 0x%llx/0x%lx 0x%lx addr: 0x%lx\n",
        bname, (long long)bn, bcount, blen, bp->b_addr); 

    unsigned long cksum_offset = -1;
    if (!strcmp(bname, "xfs_sb"))
      cksum_offset = XFS_SB_CRC_OFF;
    else if (!strcmp(bname, "xfs_agf"))
      cksum_offset = XFS_AGF_CRC_OFF;
    else if (!strcmp(bname, "xfs_agi"))
      cksum_offset = XFS_AGI_CRC_OFF;
    else if (!strcmp(bname, "xfs_agfl"))
      cksum_offset = XFS_AGFL_CRC_OFF;
    else if (!strcmp(bname, "xfs_allocbt") 
        || !strcmp(bname, "xfs_rmapbt")
        || !strcmp(bname, "xfs_refcountbt")
        || !strcmp(bname, "xfs_inobt"))
      cksum_offset = XFS_BTREE_SBLOCK_CRC_OFF;
    else if (!strcmp(bname, "xfs_inode")) {
      for (unsigned long i = 0; i < blen * 0x200 / mp->m_sb.sb_inodesize; i++) {
        unsigned long inode_bn = bn + i;
        unsigned long inode_blen = mp->m_sb.sb_inodesize / 0x200;
        cksum_offset = XFS_DINODE_CRC_OFF;
        if (fwrite(&inode_bn, 8, 1, outcrc) != 1 
          || fwrite(&inode_blen, 8, 1, outcrc) != 1
          || fwrite(&cksum_offset, 8, 1, outcrc) != 1) {
          print_warning("error writing to file: %s", strerror(errno));
          return -errno;
        }
      }
      ...
      print_warning("cannot handle field %s\n", bname);
      return -1;
    }

    if (fwrite(&bn, 8, 1, outcrc) != 1 
        || fwrite(&blen, 8, 1, outcrc) != 1
        || fwrite(&cksum_offset, 8, 1, outcrc) != 1) {
      print_warning("error writing to file: %s", strerror(errno));
      return -errno;
    }
...
_("Could not discern log; image will contain unobfuscated metadata in log."));
    ...
    // printf("log buf bb: %ld buf blen: %ld\n", iocur_top->bb, iocur_top->blen);
	...
	char 		**argv)
{
    ...
    outcrc = fopen(argv[optind + 1], "wb");
		if (outf == NULL) {
			print_warning("cannot create crcinfo file");
			goto out;
		}
	}

...
int main(int argc, char *argv[]) 
{
  ...
  if (!strcmp(argv[1], "test")) {
    ...
    f2fs_fuzzer.compress(in_path, image_buffer, meta_path);
    ...
    f2fs_fuzzer.decompress(tmp_buffer, st.st_size, true);
    ...
    void *image_buffer = malloc(st.st_size);
    f2fs_fuzzer.compress(in_path, image_buffer, "f2fs_meta.img");

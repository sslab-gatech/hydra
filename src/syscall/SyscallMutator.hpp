...
class LseekMutator : public SyscallMutator {
    ...
    public:
        ...
        virtual Syscall* createTarget(const ArgMap &argMap) {
            ...
            syscall->add_arg(new Arg(whence_options[rand32(0, sizeof(whence_options))], 0));
        ...
        virtual uint32_t havoc(void) { 
            ...
            target->args[2]->value = whence_options[rand32(0,whence.max+1)];
            return 0;
        }
...
class MkdirMutator : public SyscallMutator {
    ...
    public:
        ...
        virtual Syscall *createTarget(const ArgMap &argMap) {
            ...
            program->add_syscall(ret);
            FileObject *new_dir = new FileObject(I_DIR);
...
class RmdirMutator : public SyscallMutator {
    ...
    public:
        ...
        virtual Syscall *createTarget(const ArgMap &argMap) {
			if (program->avail_dirs.empty())
				return NULL;

            Syscall *ret = new Syscall(SYS_rmdir);
            do {
                removed_dir = program->get_random_dir();	
            } while (removed_dir->rel_path == ".");
            ret->add_arg(new Arg(program->file_paths[removed_dir], 1));
...
class SetxattrMutator : public SyscallMutator {
    ...
    public:
        ...
        virtual Syscall *createTarget(const ArgMap &argMap) {
            ...
            // side effects
            target_fobj->add_xattr(name, name_len);
	...
	public:
		...
		virtual Syscall *createTarget(const ArgMap &argMap) {
			...
			if (target_fobj->xattrs.size() != 0) {
				...
				// side effects
				std::vector<BufferObject*> &xattrs = target_fobj->xattrs;
				xattrs.erase(std::remove(xattrs.begin(), xattrs.end(), old_xattr_name), xattrs.end());
			} else {

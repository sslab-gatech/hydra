...
fsfuzz_stage:
  ...
  for(stage_cur = 0; stage_cur < stage_max; stage_cur++) {
      ...
      fs_buf_len = mutate_havoc(new_buf + meta_size, MAX_FILE);

      if (common_fuzz_stuff(argv, new_buf, meta_size + fs_buf_len)) goto abandon_entry;

      // memcpy(out_buf, in_buf, len); // mutator keeps the original internally
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q:u:kb:s:e:y:")) > 0)

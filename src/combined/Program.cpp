...
Program *Program::selective_reserialize(const char *spath, const char *dpath,
				    const char *keep_syscalls)
{
	int i;
	int num_syscalls;

	Program *program = Program::deserialize(spath, true);
	if (!program)
		return NULL;

	// got the program in the memory, now time to drop syscalls
	num_syscalls = program->syscalls.size();

	if (strlen(keep_syscalls) != num_syscalls) {
		delete program;
		return NULL;
	}

	for (i = num_syscalls - 1; i >= 0; --i) {
		if (keep_syscalls[i] == '0') {
			program->syscalls.erase(program->syscalls.begin() + i);
		}
	}

	program->serialize(dpath);
	return program;
}
...
/******************* path generator 2 *****************/
std::string Program::rand_path2()
{
	std::string ret = "./" + random_string(8);
	return ret;
}
